...
const SYSTEMS: SopSystem[] = [
{
  ...
  subflows:[
    ...
    { id:'providerid', title:'ProviderID MOPH Health ID', nodes:[
      {id:'s',  r:0,c:0,t:'start',   l:'คลิกปุ่ม ProviderID'},
      {id:'n1', r:0,c:1,t:'process', l:'GET /auth/providerid/start'},
      {id:'n2', r:0,c:2,t:'process', l:'อ่าน providerid_login_url\nจาก Settings (DB)'},
      {id:'n3', r:2,c:2,t:'process', l:'Redirect →\nproviderid_login_url\n(URL ตั้งค่าไว้)'},
      {id:'n4', r:2,c:3,t:'process', l:'ผู้ใช้ Login\nบน MOPH Portal\n(บัตรประชาชน)'},
      {id:'n5', r:0,c:4,t:'process', l:'/authen/healthid/callback\n(redirect_uri ใน URL)'},
      {id:'n6', r:0,c:5,t:'process', l:'POST token_url\ncode → access_token\nGET userinfo → CID'},
      {id:'n7', r:0,c:6,t:'process', l:'cid → SHA-256 hash'},
      {id:'n8', r:1,c:6,t:'db',      l:'Lookup users.cid\n(SHA-256 hash match)'},
      {id:'d1', r:0,c:7,t:'decision',l:'พบ User\n+ อนุมัติแล้ว?'},
      {id:'n9', r:0,c:8,t:'process', l:'ออก KHUPS JWT 8h\nsession + logs\nsendLoginNotifications\n(ProviderID)'},
      {id:'ok', r:1,c:8,t:'end',     l:'→ /sso-callback\n?sso_provider=providerid'},
      {id:'e1', r:2,c:7,t:'end',     l:'→ /sso-callback\n?sso_error=ไม่พบ/รออนุมัติ'},
    ], edges:[
      {f:'s', t:'n1'},
      {f:'n1',t:'n2'},
      ...
      {f:'d1',t:'e1',x:'bottom',n:'top',l:'ไม่'},
      {f:'n9',t:'ok',x:'bottom',n:'top'},
    ]},
  ],
  notes:[
    'JWT หมดอายุใน 8 ชั่วโมง | sessionId ตรวจสอบทุก request (30s cache)',
    'Login ซ้อน > 5 นาที = 409 Concurrent Login → บังคับ logout',
    'ThaiD token HS256 signed ด้วย thaid_client_secret ที่ตั้งไว้ใน Settings',
    'ProviderID: ตั้ง URL ปุ่มใน Settings (providerid_login_url) — redirect ตรงสู่ MOPH Portal',
    'Login สำเร็จ → sendLoginNotifications: Local DB (icon fa-user-md cyan) + LINE + Email + Telegram (4 ช่องทาง)',
  ],
},{
