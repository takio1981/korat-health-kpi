...
const SYSTEMS: SopSystem[] = [
{
  ...
  modules: [
    ...
    {id:'roleaccess',icon:'🔑', title:'สิทธิ์การเข้าถึง', desc:'Role × Page/Action Access'},
    {id:'online',  icon:'🟢', title:'ผู้ใช้งานออนไลน์', desc:'Online Users Tracking'},
    {id:'export',  icon:'💾', title:'Export & KHD',    desc:'Export + Sync KHD Wizard'},
  ...
  subflows:[
    ...
    { id:'providerid', title:'ProviderID MOPH Health ID (Direct JWT)', nodes:[
      {id:'s',  r:0,c:0,t:'start',   l:'คลิกปุ่ม ProviderID'},
      {id:'n1', r:0,c:1,t:'process', l:'sessionStorage\nsso_intent=providerid'},
      {id:'n2', r:0,c:2,t:'process', l:'GET /auth/providerid/start\nอ่าน providerid_login_url'},
      {id:'n3', r:2,c:2,t:'process', l:'Redirect →\nMOPH Portal\n(ผู้ใช้ Login\nบัตรประชาชน)'},
      {id:'n4', r:0,c:3,t:'process', l:'MOPH redirect\n→ /login?token=JWT'},
      {id:'n5', r:1,c:3,t:'process', l:'handleThaidTokenParam()\nอ่าน sso_intent\n= providerid'},
      {id:'n6', r:1,c:4,t:'process', l:'POST /auth/thaid/verify-token\nhint_provider=providerid'},
      {id:'n7', r:1,c:5,t:'process', l:'decode JWT\nextract cid → SHA-256\n(detect by iss + hint)'},
      {id:'n8', r:2,c:5,t:'db',      l:'Lookup users.cid\n(SHA-256 match)'},
      {id:'d1', r:1,c:6,t:'decision',l:'พบ User\n+ อนุมัติ?'},
      {id:'n9', r:1,c:7,t:'process', l:'ออก KHUPS JWT 8h\nsendLoginNotifications\n(ProviderID)'},
      {id:'ok', r:0,c:7,t:'end',     l:'Swal "ProviderID"\n→ /dashboard'},
      {id:'e1', r:2,c:6,t:'end',     l:'→ /login\n?sso_error=ไม่พบ\n/รออนุมัติ'},
    ], edges:[
      {f:'s', t:'n1'},
      {f:'n1',t:'n2'},
  ...
  notes:[
    'JWT หมดอายุใน 8 ชั่วโมง | sessionId ตรวจสอบทุก request (30s cache)',
    'Login ซ้อน > 5 นาที = 409 Concurrent Login → บังคับ logout',
    'ThaID + ProviderID ใช้ Direct JWT Flow เหมือนกัน — ทั้งคู่ redirect มาที่ /login?token=JWT',
    'ProviderID: ตั้งเฉพาะ providerid_login_url ใน Settings — ไม่ต้องกรอก OAuth config เพิ่ม',
    'Provider detection: ใช้ sessionStorage sso_intent (hint_provider) OR JWT iss field (auto-detect)',
    'สถานะ connectivity ของปุ่ม ThaID/ProviderID แสดงแบบ realtime ด้วย GET /sso/connectivity (server-side HEAD check)',
    'sendLoginNotifications แสดง login method ที่ถูกต้อง: Username/Password | ThaID | ProviderID — ทุกช่องทาง (LINE/Telegram/Email/in-app)',
  ],
},{
  ...
  notes:['Focus Mode: เข้าแก้ไข → sidebar ซ่อน (focusMode$ BehaviorSubject)','admin_ssj + super_admin ไม่โหลดอัตโนมัติ ต้องกด "ค้นหา"','applySubSummaryToKpiData() early-return ถ้า isEditing=true','ตัวชี้วัดสะสม (is_cumulative=1): badge "สะสม" สีม่วงต่อชื่อตัวชี้วัด — ผลงานล่าสุด = SUM ทุกเดือนที่มีค่าในปีงบ แทนค่าเดือนล่าสุดปกติ'],
},{
  ...
  notes:['Bulk Import: แสดง แถว+คอลัมน์ที่ error ชัดเจน | import แถวถูกต้อง skip แถว error','Form Builder embed: กด clipboard icon → modal สร้าง/แก้ schema (super_admin)','evaluation_mode: any_one (บางประเภท) | all_required (บังคับทุกประเภท) — ตั้งค่าที่ section "เกณฑ์การประเมิน" ในโมดัล','Modal เพิ่ม/แก้ไข: 2 กล่องแยกกัน — "เป้าหมาย" (target_percentage, ค่าเริ่มต้น copy ครั้งเดียวตอนตั้งปีงบใหม่/เพิ่ม KPI ให้หน่วยบริการ) กับ "เกณฑ์" (criterion+target_condition, ใช้เทียบกับ KHD ทั่วระบบผ่าน criteriaText pipe) — คนละความหมาย ห้ามสลับกัน','checkbox "ตัวชี้วัดสะสม" ในกล่อง "เกณฑ์" → เปิดแล้ว Dashboard แสดง badge "สะสม" สีม่วง (ดูระบบบันทึก KPI)'],
},{
  ...
  desc:'CRUD ผู้ใช้งาน อนุมัติ บังคับ logout | ซิงค์กับ KHD | Force Logout ทำงานใน ~30 วินาที',
  ...
  nodes:[
    ...
    {id:'ae', r:2,c:3,t:'decision',l:'Data Sync\nเลือกแท็บ?'},
    {id:'n3', r:0,c:3,t:'db',      l:'PUT /users/:id/approve\nis_approved=1'},
    {id:'n4', r:3,c:2,t:'process', l:'POST /admin/force-logout\nclear active_session_id'},
    {id:'af', r:1,c:4,t:'process', l:'เปรียบเทียบข้อมูล\n4 สถานะ + field diff'},
    {id:'ag', r:2,c:4,t:'process', l:'ตรวจโครงสร้างตาราง\n(รายงานอย่างเดียว)'},
    {id:'ah', r:3,c:4,t:'process', l:'ตั้งค่า Mapping\ncolumn + exclude list'},
    {id:'n5', r:1,c:5,t:'process', l:'POST /users/sync-to-khd\nUPSERT batch 100 (ตาม mapping)'},
    {id:'n5b',r:2,c:5,t:'db',      l:'GET /users/structure-compare\ndiffTableColumns()'},
    {id:'n5c',r:3,c:5,t:'db',      l:'system_settings:\nusers_sync_field_mapping/exclude'},
    {id:'ok', r:0,c:7,t:'end',     l:'อัปเดตสำเร็จ'},
  ],
  edges:[
    {f:'s', t:'n1',x:'bottom',n:'top'},
    {f:'n1',t:'n2',x:'bottom',n:'top'},
    ...
    {f:'ae',t:'af',x:'top',n:'bottom',l:'เปรียบเทียบ'},
    {f:'ae',t:'ag',l:'โครงสร้าง'},
    {f:'ae',t:'ah',x:'bottom',n:'top',l:'Mapping'},
    {f:'af',t:'n5',x:'right',n:'left'},
    {f:'ag',t:'n5b',x:'right',n:'left'},
    {f:'ah',t:'n5c',x:'right',n:'left'},
    {f:'n3',t:'ok'},
    {f:'n4',t:'ok',x:'top',n:'bottom',v:'hv'},
    {f:'n5',t:'ok',x:'top',n:'bottom',v:'hvh'},
    {f:'n5b',t:'ok',x:'right',n:'left',v:'hv'},
    {f:'n5c',t:'ok',x:'top',n:'bottom',v:'hvh'},
  ],
  notes:['Force Logout → ผู้ถูก logout จะถูกตัด session ใน ~30 วินาที (session cache TTL)','Sync users to KHD: 3 แท็บ — เปรียบเทียบข้อมูล (4 สถานะ) / ตรวจโครงสร้างตาราง (รายงานอย่างเดียว) / ตั้งค่า Mapping คอลัมน์+exclude list'],
},{
  id:'roleaccess', icon:'🔑', title:'ระบบสิทธิ์การเข้าถึง', sub:'Role × Page/Action Access',
  desc:'Super Admin กำหนดสิทธิ์เข้าถึงหน้า (8 role × 19 หน้า) + สิทธิ์เพิ่ม/แก้ไขข้อมูล (8 role × 3 หน้า × 2 action) แบบไดนามิก ไม่ต้องแก้โค้ด',
  actors:['Super Admin','Backend API','ฐานข้อมูล'],
  subflows:[
    { id:'page', title:'สิทธิ์เข้าถึงหน้า (Page Access)', nodes:[
      {id:'s',  r:0,c:0,t:'start',   l:'เปิด /role-page-access\n(super_admin)'},
      {id:'n1', r:0,c:1,t:'process', l:'GET /role-page-access\nmatrix 8 role × 19 หน้า'},
      {id:'n2', r:0,c:2,t:'process', l:'Toggle checkbox\nเปิด/ปิดต่อ role×page'},
      {id:'n3', r:0,c:3,t:'process', l:'PUT /role-page-access'},
      {id:'n4', r:2,c:3,t:'db',      l:'UPDATE role_page_access\nis_enabled'},
      {id:'n5', r:1,c:4,t:'process', l:'reload\n_rolePageAccessCache'},
      {id:'n6', r:1,c:5,t:'process', l:'authenticateToken เช็ค\nPAGE_ACCESS_RULES ทุก request'},
      {id:'n7', r:0,c:5,t:'process', l:'GET /my-page-access\n(ทุก role หลัง login)'},
      {id:'n8', r:0,c:6,t:'process', l:'canAccessPage()\nlocalStorage cache'},
      {id:'ok', r:0,c:7,t:'end',     l:'sidebar + guard\nกรองตามสิทธิ์'},
    ], edges:[
      {f:'s', t:'n1'},
      {f:'n1',t:'n2'},
      {f:'n2',t:'n3'},
      {f:'n3',t:'n4',x:'bottom',n:'top',v:'vh'},
      {f:'n4',t:'n5',x:'top',n:'bottom',v:'hv'},
      {f:'n5',t:'n6',x:'top',n:'bottom'},
      {f:'n6',t:'n7',x:'top',n:'bottom',v:'vh'},
      {f:'n7',t:'n8'},
      {f:'n8',t:'ok'},
    ]},
    { id:'action', title:'สิทธิ์เพิ่ม/แก้ไขข้อมูล (Action Access)', nodes:[
      {id:'s',  r:0,c:0,t:'start',   l:'ตารางที่ 2\nหน้า /role-page-access เดียวกัน'},
      {id:'n1', r:0,c:1,t:'process', l:'GET /role-action-access\n3 หน้า × 2 action (เพิ่ม/แก้ไข)'},
      {id:'n2', r:0,c:2,t:'process', l:'Toggle checkbox\nต่อ role/page/action'},
      {id:'n3', r:0,c:3,t:'process', l:'PUT /role-action-access'},
      {id:'n4', r:2,c:3,t:'db',      l:'UPDATE role_action_access'},
      {id:'n5', r:1,c:4,t:'process', l:'reload\n_roleActionAccessCache'},
      {id:'n6', r:1,c:5,t:'process', l:'requireAction(page,action)\nแทนที่ isSuperAdmin เดิม'},
      {id:'n7', r:0,c:5,t:'process', l:'GET /my-action-access\n(ทุก role)'},
      {id:'n8', r:0,c:6,t:'process', l:'canPerformAction()\ngate ปุ่ม เพิ่ม/แก้ไข'},
      {id:'ok', r:0,c:7,t:'end',     l:'ปุ่มแสดง/ซ่อน\nตามสิทธิ์'},
    ], edges:[
      {f:'s', t:'n1'},
      {f:'n1',t:'n2'},
      {f:'n2',t:'n3'},
  ...
  notes:[
    'Page Access ครอบคลุม 19 หน้า | Action Access ครอบคลุมแค่ 3 หน้าที่ปลอดภัยพอ: kpi-manage / users / announcements (เฉพาะเพิ่ม+แก้ไข)',
    'DELETE ทุกจุดยัง hardcode super_admin เสมอ ไม่มีข้อยกเว้น — Action Access ไม่แตะ DELETE เลย',
    '2 ระบบนี้เป็นเช็คอิสระ ไม่ผูกกัน — บางหน้า (เช่น users) ต้องผ่านทั้ง Page Access และ Action Access คนละชั้น',
    'super_admin ข้ามทั้ง 2 ระบบนี้เสมอ (เข้าได้/ทำได้ทุกอย่าง) | localStorage cache แบบ fail-open กันเมนูหาย ฝั่ง backend เป็นด่านตัดสินจริง',
  ],
},{
  id:'online', icon:'🟢', title:'ระบบติดตามผู้ใช้งานออนไลน์', sub:'Online Users Tracking',
  desc:'ติดตามผู้ใช้ที่ active แบบ realtime จาก last_seen_at ที่อัปเดตทุก request (throttled)',
  actors:['ผู้ใช้งาน','Super Admin','Backend API','ฐานข้อมูล'],
  nodes:[
    {id:'s',  r:0,c:0,t:'start',   l:'ผู้ใช้งานทุกคน\nใช้งานระบบ (ทุก request)'},
    {id:'n1', r:2,c:0,t:'process', l:'authenticateToken\nUPDATE last_seen_at+ip+ua'},
    {id:'n2', r:3,c:0,t:'db',      l:'users.last_seen_at\n(throttle 1 ครั้ง/นาที/user)'},
    {id:'s2', r:1,c:1,t:'start',   l:'Super Admin\nเปิด /online-users'},
    {id:'n3', r:2,c:1,t:'process', l:'GET /online-users?window=N\nfilter role + 1-1440 นาที'},
    {id:'n4', r:3,c:1,t:'db',      l:'SELECT users\nWHERE last_seen_at >= NOW()-N min'},
    {id:'n5', r:1,c:2,t:'process', l:'คำนวณ idle_seconds\n+ status dot สี'},
    {id:'n6', r:1,c:3,t:'process', l:'auto-refresh\n5-60 วินาที'},
    {id:'ok', r:1,c:4,t:'end',     l:'ตาราง realtime\nonline users'},
  ],
  edges:[
    {f:'s', t:'n1',x:'bottom',n:'top',v:'vh'},
    {f:'n1',t:'n2',x:'bottom',n:'top'},
    {f:'n2',t:'n4',x:'right',n:'left',v:'hv'},
    {f:'s2',t:'n3',x:'bottom',n:'top',v:'vh'},
    {f:'n3',t:'n4',x:'bottom',n:'top'},
    {f:'n4',t:'n5',x:'top',n:'bottom',v:'hv'},
    {f:'n5',t:'n6'},
    {f:'n6',t:'ok'},
  ...
  actors:['Super Admin','Backend API','ฐานข้อมูล','KHD Remote DB'],
  nodes:[
    {id:'s',  r:0,c:0,t:'start',   l:'เปิด KPI Manager'},
    {id:'w1', r:0,c:1,t:'process', l:'Step 1: DB Compare\nSchema + 4 operations'},
    {id:'w2', r:0,c:2,t:'process', l:'Step 2: Export\nKPI Tables'},
    ...
    {id:'n4', r:1,c:7,t:'process', l:'performSyncToKhd()\nINSERT ON DUPLICATE'},
    {id:'n5', r:3,c:7,t:'db',      l:'KHD Remote DB\n(ไม่ลบข้อมูลเดิม)'},
  ...
  notes:['Report Compare (เทียบ + เพิ่มตัวชี้วัดจาก KHD) ย้ายไปอยู่ในหน้า "จัดการตัวชี้วัด" แล้ว (ปุ่ม "เทียบกับ KHD" ต่อแท็บ) ไม่ได้อยู่ใน Wizard นี้','Scheduler: setInterval 30s, match 2-min window, atomic lock via UPDATE last_run_at','result = last actual value (ค่าเดือนล่าสุด) ไม่ใช่ SUM | content-based diff ไม่ใช่ timestamp'],
},{
  id:'notif', icon:'🔔', title:'ระบบแจ้งเตือนและประกาศ', sub:'Notifications & Announcements',
  desc:'4 ช่องทาง: Local DB | LINE (group admin + personal user) | Email | Telegram — KPI events + SSO login + Announcements',
  actors:['ผู้ใช้งาน','Admin / Super Admin','Backend API','ฐานข้อมูล'],
  subflows:[
    { id:'kpi', title:'KPI / Register Notifications', nodes:[
      {id:'s',  r:2,c:0,t:'start',   l:'เหตุการณ์ KPI\nหรือ Register'},
      ...
      {id:'n1', r:2,c:3,t:'db',      l:'[1] Local DB\nINSERT notifications'},
    ...
    { id:'login', title:'SSO Login Notifications (4 ช่องทาง)', nodes:[
      {id:'s',  r:0,c:0,t:'start',   l:'Login สำเร็จ\nThaiD / ProviderID'},
      {id:'n0', r:1,c:0,t:'process', l:'sendLoginNotifications\n(user, ip, ua, provider)'},
      {id:'n1', r:0,c:1,t:'db',      l:'[1] Local DB\nINSERT notifications\n(ทุก user)'},
      {id:'d1', r:1,c:1,t:'decision',l:'Admin role?'},
      {id:'n2', r:0,c:2,t:'process', l:'[2] LINE group\nnotifyLineAction\n(admin_login)'},
      {id:'n3', r:0,c:3,t:'process', l:'[5] Telegram\nsendTelegramDirect\n(admin group)'},
      {id:'n4', r:2,c:2,t:'process', l:'[3] LINE personal\nsendLineToUser\n(ถ้าผูก line_user_id)'},
      {id:'d2', r:3,c:2,t:'decision',l:'มี email\nในระบบ?'},
      {id:'n5', r:3,c:3,t:'process', l:'[4] Email\nsendMail\n(login alert HTML)'},
      {id:'ok', r:0,c:4,t:'end',     l:'แจ้งเตือนส่งแล้ว\n(fire & forget)'},
    ], edges:[
      {f:'s',  t:'n0',x:'bottom',n:'top'},
      {f:'n0', t:'n1',x:'top',n:'bottom',v:'vh'},
      {f:'n0', t:'d1',x:'right',n:'left'},
      {f:'d1', t:'n2',x:'top',n:'bottom',l:'ใช่'},
      {f:'d1', t:'n4',x:'bottom',n:'top',l:'ไม่'},
      {f:'n2', t:'n3'},
      {f:'n3', t:'ok'},
      {f:'n4', t:'d2',x:'bottom',n:'top'},
      {f:'d2', t:'n5',x:'right',n:'left',l:'มี'},
      {f:'d2', t:'ok',x:'right',n:'bottom',v:'hv',l:'ไม่มี'},
      {f:'n5', t:'ok',x:'top',n:'bottom',v:'vh'},
    ]},
  ],
  notes:[
    '4 ช่องทาง: [1] Local DB (ทุก user) [2] LINE group admin [3] LINE personal user [4] Email [5] Telegram admin',
    'LINE group + Telegram: เฉพาะ super_admin/admin_ssj | LINE personal: ทุก user ที่ผูก line_user_id',
    'unreadCount$ BehaviorSubject ใน AuthService ← layout poll ทุกครั้งที่ load',
    'show_on_header=1 → banner ติดทุกหน้า | show_on_login=1 → แสดงบน login page',
  ],
},{
...
export class SopComponent implements AfterViewInit, OnDestroy {
  ...
  private renderOverview(wrap: HTMLElement, sys: SopSystem) {
    ...
    const rows: [string, string][] = [
      ...
      ['💾 Export', 'Export tables → KHD | Refresh kpi_summary → Charts ทันสมัย'],
      ...
      ['🔑 สิทธิ์การเข้าถึง', 'ควบคุมว่า Role ไหนเห็นเมนู/หน้าไหนบ้าง + เพิ่ม/แก้ไขได้ไหม — คุมทุกระบบข้างบน'],
      ['🟢 ผู้ใช้งานออนไลน์', 'อ่าน last_seen_at ที่ Auth อัปเดตทุก request — ไม่ผูกกับระบบอื่น'],
    ];
