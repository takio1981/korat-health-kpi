...
const SYSTEMS: SopSystem[] = [
{
  ...
  subflows:[
    ...
    { id:'providerid', title:'ProviderID MOPH Health ID', nodes:[
      ...
      {id:'n2', r:1,c:1,t:'process', l:'getProviderIdSettings()\nสร้าง CSRF state 10min'},
      {id:'n3', r:3,c:1,t:'process', l:'Redirect →\nmoph.id.th/oauth/redirect\n?client_id=...&state='},
      {id:'n4', r:3,c:2,t:'process', l:'ผู้ใช้ Login\nbน MOPH Portal\n(บัตรประชาชน)'},
      {id:'n5', r:1,c:3,t:'process', l:'/authen/healthid/callback\n(redirect_uri จดทะเบียน)'},
      {id:'n6', r:1,c:4,t:'process', l:'Validate CSRF state\nPOST token_url\ncode → access_token'},
      {id:'n7', r:3,c:4,t:'process', l:'GET userinfo_url\nextract CID (13 หลัก)\ncid → SHA-256 hash'},
      {id:'n8', r:2,c:5,t:'db',      l:'Lookup users.cid\n(SHA-256 hash match)'},
      {id:'d1', r:1,c:5,t:'decision',l:'พบ User\n+ อนุมัติแล้ว?'},
      {id:'n9', r:1,c:6,t:'process', l:'ออก KHUPS JWT 8h\nsession + logs\nsendLoginNotifications'},
      {id:'ok', r:0,c:7,t:'end',     l:'→ /sso-callback\n?sso_token=JWT\n?sso_provider=providerid'},
      {id:'e1', r:3,c:5,t:'end',     l:'Redirect /sso-callback\n?sso_error=\nไม่พบ/รออนุมัติ'},
    ], edges:[
  ...
  notes:[
    ...
    'ProviderID auth_url: moph.id.th/oauth/redirect | callback: /authen/healthid/callback',
    'Login สำเร็จ → sendLoginNotifications: Local DB + LINE + Email + Telegram (4 ช่องทาง)',
  ],
},{
