  ...
  changelog = [
    {
      ...
      changes: [
        { type: 'feature', text: 'เปลี่ยนคำที่แสดงในหน้าจอเกี่ยวกับ "upload_excel"/"Excel มือเอง" ให้ใช้คำว่า "key_in" แทนทั้งหมด (หน้าจัดการตัวชี้วัด, จัดการข้อมูล KPI, คู่มือการใช้งาน) — เฉพาะข้อความที่แสดงผล ไม่กระทบชื่อ column ในฐานข้อมูลหรือ API เดิม' },
        { type: 'feature', text: 'เพิ่ม badge แสดงแหล่งที่มาข้อมูลให้ทุกตัวชี้วัด ทั้งในหน้า "จัดการตัวชี้วัด" และหน้า "บันทึกผลงานตัวชี้วัด" — แบ่ง 3 สถานะ: key_in (คีย์ข้อมูลเอง), hdc (ดึงข้อมูลอัตโนมัติจาก HDC), local-only (ไม่พบเทียบเคียงกับ KHD)' },
        { type: 'feature', text: 'เพิ่มคอลัมน์ khd_report_id ในตาราง kpi_indicators เก็บลิงก์ถาวรไปยัง KHD reports.report_id (จับคู่ผ่าน table_process) พร้อมปุ่ม "เชื่อมโยง KHD Link" ในหน้าจัดการตัวชี้วัด (super_admin) สำหรับดึงข้อมูล data_source จริงจาก KHD มาเก็บไว้ถาวรให้ครบทุกตัวชี้วัด (ก่อนหน้านี้มีข้อมูลจริงแค่ 2 จาก 172 ตัวชี้วัด เพราะเทียบแบบชั่วคราวเฉพาะตอนเปิดหน้าเท่านั้น)' }
      ]
    },
    {
      version: '2569.09.26.f',
      date: '26 กันยายน 2569 (build f)',
      changes: [
        { type: 'fix', text: 'แก้ modal เพิ่ม/แก้ไขข้อมูล (เช่น modal "แก้ไขข้อมูล" ในหน้าจัดการตัวชี้วัด) แสดงผลไม่ครบ ส่วนบนถูก header ด้านบนของระบบบังอยู่ — เป็นผลข้างเคียงจากการแก้บั๊กเมนู slide มือถือก่อนหน้านี้ที่ยกระดับ header ให้อยู่เหนือเนื้อหาเสมอ ทำให้ไปทับ modal ทั่วทั้งระบบที่ใช้ระดับชั้นเดียวกันโดยไม่ตั้งใจ แก้โดยให้ header ยกระดับเฉพาะตอนเปิดเมนู slide มือถือจริงๆ เท่านั้น ปกติแล้วจะอยู่ระดับเดิมเหมือนก่อนแก้บั๊กเมนู slide' }
      ]
    },
    {
      version: '2569.09.26.e',
      date: '26 กันยายน 2569 (build e)',
      changes: [
        { type: 'fix', text: 'แก้บั๊กในหน้า "จัดการตัวชี้วัด" — ปุ่ม "เทียบกับ KHD" เดิมใช้ค่า is_active ทั่วไปของ KHD (ไม่ผูกปีงบ) ในการคำนวณ badge "HDC inactive" และคำแนะนำปิดส่งออกอัตโนมัติ ทั้งที่ควรใช้ค่า is_active เฉพาะปีงบที่กำลังเทียบ (จาก report_fiscal_year_config) ทำให้บางตัวชี้วัดแสดงสถานะผิดเมื่อ 2 ค่านี้ไม่ตรงกัน (ยืนยันพบจริงในข้อมูล) แก้ไขแล้ว พร้อมเพิ่มตัวเลือกปีงบให้เลือกเทียบปีอื่นที่ไม่ใช่ปีปัจจุบันได้ (เดิมเทียบได้แค่ปีปัจจุบันเท่านั้นเพราะไม่เคยส่งปีงบไปให้ backend เลย)' },
        { type: 'feature', text: 'เพิ่มหน้าใหม่ "จัดการข้อมูลผลงานตัวชี้วัด" (super_admin เท่านั้น) — ดูและลบข้อมูลผลงานตัวชี้วัดที่บันทึกแล้วแบบละเอียดถึงระดับรายการเดียว (1 ตัวชี้วัด × 1 หน่วยบริการ × 1 เดือน) กรองได้ทั้งช่วงวันที่บันทึกจริงและช่วงปีงบ/เดือนของผลงาน เลือกลบได้ทีละรายการหรือเลือกทั้งหมดในหน้า ระบบล้างข้อมูลที่เกี่ยวข้อง (ผลงานย่อย + ตารางสรุป) ให้อัตโนมัติหลังลบ' }
      ]
    },
    {
      version: '2569.09.26.d',
      date: '26 กันยายน 2569 (build d)',
      changes: [
        { type: 'improve', text: 'ปรับข้อความแจ้งเตือนตอนทดสอบส่ง LINE ไม่สำเร็จ (ตั้งค่าระบบ/ทดสอบ LINE ส่วนตัว/แอดมินทดสอบให้ผู้ใช้) ให้บอกสาเหตุจริงจาก LINE โดยตรง เช่น "โควตาข้อความเดือนนี้เต็มแล้ว", "Token หมดอายุ", "userId ไม่ถูกต้อง/ยังไม่ได้ Add friend" แทนข้อความเดิมที่บอกให้ "ตรวจสอบ Channel Access Token และ Group ID" เสมอไม่ว่าสาเหตุจริงจะเป็นอะไร ซึ่งทำให้เข้าใจผิดได้เมื่อ Token/Group ID ถูกต้องอยู่แล้วแต่ปัญหาจริงคือโควตาบัญชี LINE เต็ม' }
      ]
    },
    {
      version: '2569.09.26.c',
      date: '26 กันยายน 2569 (build c)',
      changes: [
        { type: 'fix', text: 'แก้เมนู slide บนมือถือ บางหน้าบางครั้งกดปุ่มแฮมเบอร์เกอร์แล้วเมนูไม่ขึ้นมาเลย — สาเหตุคือระบบใช้ตัวแปรตัวเดียวกันควบคุมทั้ง "เมนู slide บนมือถือ" และ "การเปิด/ปิด sidebar บนจอใหญ่" ซึ่งโหมด "แก้ไขผลงาน" (โหมดเต็มพื้นที่) มีกลไกจำ/คืนค่าตัวแปรนี้อยู่แล้วสำหรับจอใหญ่ พอมาแชร์ตัวแปรเดียวกันบนจอมือถือ ทำให้บางจังหวะการเข้า-ออกโหมดแก้ไขไปรบกวนสถานะเมนู slide จนไม่ตรงกับปุ่มที่กดจริง แก้โดยแยกตัวแปรควบคุมเมนู slide มือถือออกมาต่างหาก ไม่ให้โหมดแก้ไขไปยุ่งเกี่ยวด้วยอีกต่อไป' }
      ]
    },
    {
      version: '2569.09.26.b',
      date: '26 กันยายน 2569 (build b)',
      changes: [
        { type: 'fix', text: 'แก้เมนู slide บนมือถือ (หน้าจอแคบ) กดเลือกเมนูไม่ได้ในบางหน้า เช่น "บันทึกผลงานตัวชี้วัด" — สาเหตุคือกล่องตัวกรองของหน้านั้น (และหน้าอื่นที่มีหัวตาราง sticky แบบเดียวกัน) ใช้ z-index เท่ากับเมนู slide พอดี ทำให้กล่องตัวกรองซึ่งอยู่หลังในโครงสร้างหน้าเว็บทับเมนูจนกดไม่ทะลุ แก้โดยยกระดับ z-index ของเมนู slide + ฉากหลังมืดให้อยู่เหนือทุกหน้าเสมอเมื่อเปิดใช้งาน' }
      ]
    },
    {
      version: '2569.09.26.a',
      date: '26 กันยายน 2569 (build a)',
      changes: [
        { type: 'improve', text: 'อัปเดตผังกระบวนการทำงาน (SOP) และคู่มือการใช้งาน (Help) ให้ตรงกับระบบปัจจุบัน — เพิ่มผังใหม่ 2 ระบบที่ขาดหายไป ("สิทธิ์การเข้าถึง" แยกหน้า/แยกการเพิ่ม-แก้ไข และ "ผู้ใช้งานออนไลน์"), ขยายผัง "Data Synchronization (Users)" ให้ครบทั้ง 3 แท็บ (เปรียบเทียบข้อมูล/ตรวจโครงสร้างตาราง/ตั้งค่า Mapping) แทนผังเดิมที่มีแค่ sync ทางเดียว, แก้ผัง "จัดการข้อมูล KPI" จาก Wizard 3 ขั้น (มี Report Compare) เป็น 2 ขั้นตามจริง (Report Compare ย้ายไปอยู่หน้าจัดการตัวชี้วัดแล้ว), เพิ่มคำอธิบาย "เป้าหมาย vs เกณฑ์" และ badge "สะสม" ที่ขาดหายไปทั้งใน SOP และคู่มือ, และแก้คำถามที่พบบ่อยในคู่มือที่ยังบอกว่ามี 4 แท็บซึ่งขัดกับเนื้อหาจริงในหน้าเดียวกัน' }
      ]
    },
    {
      version: '2569.09.25.k',
      date: '25 กันยายน 2569 (build k)',
      changes: [
        { type: 'improve', text: 'เปลี่ยนชื่อการเชื่อมต่อฐานข้อมูลภายนอกที่ใช้เปรียบเทียบ/ซิงค์ข้อมูลตัวชี้วัดจาก "HDC" เป็น "KHD" ทั้งระบบ — ครอบคลุมทุกหน้าที่มีฟีเจอร์นี้ (จัดการตัวชี้วัด, เปรียบเทียบรายงาน, เปรียบเทียบโครงสร้างฐานข้อมูล, จัดการผู้ใช้งาน (Data Sync), Export ตัวชี้วัด, จัดการข้อมูล KPI, สร้างฟอร์ม, คู่มือการใช้งาน) รวมถึง endpoint path, ชื่อตัวแปรสภาพแวดล้อม (env var) และคอลัมน์ฐานข้อมูล 2 ตัว (kpi_indicators.hdc_fiscal_year → khd_fiscal_year, export_schedules.auto_sync_hdc → auto_sync_khd — migrate อัตโนมัติ ข้อมูลเดิมไม่หาย) เป็นการเปลี่ยนชื่อเรียกเท่านั้น ไม่กระทบการทำงานหรือการเชื่อมต่อจริงกับฐานข้อมูลภายนอก' }
      ]
    },
    {
      version: '2569.09.25.j',
      date: '25 กันยายน 2569 (build j)',
      changes: [
        { type: 'feature', text: 'หน้า "จัดการตัวชี้วัด" ปุ่ม "เพิ่มข้อมูล"/"แก้ไข": แยกฟิลด์ "เป้าหมาย" (ค่าเริ่มต้นที่คัดลอกเข้าเป็นเป้าหมายของแต่ละหน่วยบริการตอนตั้งค่าปีงบใหม่/เพิ่ม KPI ให้หน่วยบริการ) ออกจาก "เกณฑ์" (ใช้เทียบ "เกณฑ์ต่างกัน" กับ HDC) ที่แต่ก่อนใช้ฟิลด์เดียวกันปนกันจนแก้ไขอย่างใดอย่างหนึ่งกระทบอีกอย่างโดยไม่ตั้งใจ — เพิ่มคอลัมน์ criterion ใหม่พร้อม backfill ค่าเดิมอัตโนมัติ (ตัวชี้วัดเดิมเทียบ HDC ได้ถูกต้องเหมือนเดิม ไม่ต้องกรอกใหม่) ปรับ Dashboard/รายงาน/Export/kpi-setup/Form Builder/Report Compare ให้แสดงผลตรงกับความหมายจริง' },
        { type: 'feature', text: 'หน้า "บันทึกผลงานตัวชี้วัด": เพิ่ม badge "สะสม" (สีม่วง) ต่อชื่อตัวชี้วัดที่ตั้งเป็นตัวชี้วัดสะสม (is_cumulative) แทนข้อความแจ้งเตือน static เดิมที่เหมารวมทุกแถวในตาราง (ไม่ถูกต้องสำหรับตัวชี้วัดที่ไม่ใช่สะสม)' }
      ]
    },
    {
      version: '2569.09.25.i',
      date: '25 กันยายน 2569 (build i)',
      changes: [
        { type: 'improve', text: 'หน้า "จัดการตัวชี้วัด": ปรับตำแหน่ง/ขนาดตัวกรอง "หมวดหมู่หลัก" และ "หน่วยงาน" ให้กระชับขึ้น — เดิม dropdown ทั้ง 2 ตัวไม่ได้กำหนดความกว้างไว้ ขยายตามข้อความตัวเลือกที่ยาวที่สุด (ชื่อหมวดหมู่บางรายการยาวเป็นประโยค) ทำให้ล้นไปเป็นแถวแยกต่างหากและเหลือพื้นที่ว่างด้านขวาจำนวนมาก แก้โดยกำหนดความกว้างคงที่ + ตัดข้อความยาวด้วย "..." พร้อมย้ายมาอยู่แถวเดียวกับช่องค้นหาตามลำดับ ชื่อ → หมวดหมู่หลัก → หน่วยงาน → สถานะ ทำให้ตัวกรองทั้งหมดอยู่ในแถวเดียวบนจอกว้าง (เดิมใช้ 3 แถว) พร้อมปรับ dropdown "อำเภอ/ประเภท" ในแท็บหน่วยบริการให้กระชับแบบเดียวกันเพื่อความสม่ำเสมอทั้งหน้า' },
      ]
    },
    {
      version: '2569.09.25.h',
      date: '25 กันยายน 2569 (build h)',
      changes: [
        { type: 'feature', text: 'หน้า "จัดการตัวชี้วัด" แท็บ "ตัวชี้วัด": เพิ่มตัวกรอง "หมวดหมู่หลัก" และ "หน่วยงาน" (นอกเหนือจากค้นหาชื่อ/รหัสและสถานะเปิด-ปิดที่มีอยู่เดิม) พร้อมข้อความแจ้ง "ไม่พบข้อมูลที่ตรงกับการค้นหา/ตัวกรอง" เมื่อค้นหาหรือกรองแล้วไม่พบข้อมูล (แยกจากข้อความ "ยังไม่มีข้อมูล" กรณี list ว่างจริงๆ) ใช้งานได้ทั้งมุมมองตารางเดสก์ท็อปและการ์ดมือถือ' },
      ]
    },
    {
    ...
    },
    {
      ...
      changes: [
        { type: 'feature', text: 'หน้า "Data Synchronization — Users" เพิ่ม 3 แท็บ: (1) "เปรียบเทียบข้อมูล" — แถวที่ "ต่างกัน" กดดูรายละเอียดได้ว่าฟิลด์ไหนต่างกันบ้าง เทียบค่า Local vs HDC ตรงๆ (2) "ตรวจสอบโครงสร้างตาราง" — ตรวจสอบว่าคอลัมน์ของตาราง users ระหว่าง Local กับ HDC ตรงกันไหม (3) "ตั้งค่า Mapping" — จับคู่ชื่อคอลัมน์ที่ไม่ตรงกันระหว่าง 2 ฐานข้อมูล (เช่น cid ของ Local ควร sync ไปคอลัมน์ cid_hash ของ HDC) พร้อมเลือกคอลัมน์ที่ไม่ต้องการ sync ได้' },
        { type: 'fix', text: 'แก้ไขข้อมูลที่ sync ไป HDC ไม่ถูกต้อง โดยเฉพาะ CID — ตรวจพบว่าตาราง users ของ HDC มีคอลัมน์ cid_hash แยกต่างหากจาก cid ซึ่ง Local ไม่มี ทำให้ค่า CID (hash) ที่ Local ส่งไปถูกเขียนลงคอลัมน์ cid ของ HDC แทนที่จะเป็น cid_hash ที่ควรจะเป็น แก้โดยเพิ่มระบบตั้งค่า Column Mapping ให้ Super Admin จับคู่คอลัมน์ปลายทางเองได้ (ดูแท็บ "ตั้งค่า Mapping" ด้านบน) — ค่า Role ที่เคยดูเหมือนต่างกันแบบสุ่มๆ ก็หาสาเหตุง่ายขึ้นมากเพราะตอนนี้เห็น field diff จริงชัดเจนแล้ว ไม่ปนกับคอลัมน์ session/tracking ที่เปลี่ยนตลอดเวลา (ตั้งค่า exclude list ไม่เทียบคอลัมน์เหล่านี้เป็นค่าเริ่มต้น)' },
      ]
    },
    {
      version: '2569.09.25.e',
      date: '25 กันยายน 2569 (build e)',
      changes: [
        { type: 'improve', text: 'ปุ่ม "เข้าสู่ระบบด้วย ThaID/ProviderID": ขยายขนาดโลโก้ให้ใหญ่ขึ้นพอดีกับความสูงของปุ่ม (จากเดิมดูเล็กลอยอยู่กลางปุ่ม) พร้อมปรับสีขอบ/ตัวอักษรปุ่มให้เข้าธีมสีของโลโก้แต่ละอัน — ThaID เปลี่ยนเป็นโทนน้ำเงินคราม (indigo) ให้เข้ากับพื้นวงกลมสีกรมท่าของโลโก้ และ ProviderID เปลี่ยนเป็นโทนเขียวมรกต (emerald) ให้เข้ากับตัวอักษร PROVIDER สีเขียวของโลโก้' },
      ]
    },
    {
      version: '2569.09.25.d',
      date: '25 กันยายน 2569 (build d)',
      changes: [
        { type: 'improve', text: 'ออกแบบกล่อง "เข้าสู่ระบบ" ใหม่ให้กระชับขึ้น — ปรับระยะห่าง/ขนาด padding ของช่องกรอกข้อมูล ปุ่ม และปุ่ม ThaID/ProviderID ให้พอดีอยู่ในกล่องเสมอ แก้ปัญหาเดิมที่บนหน้าจอความสูงจำกัด (เช่น โน้ตบุ๊กหน้าจอเตี้ย) ปุ่ม ProviderID และส่วนท้ายจะถูกซ่อนเลยขอบล่างของกล่องจนต้องเลื่อนหาโดยไม่รู้ตัว' },
      ]
    },
    {
      version: '2569.09.25.c',
      date: '25 กันยายน 2569 (build c)',
      changes: [
        { type: 'fix', text: 'แก้ไขปัญหาแจ้งเตือน "ผู้ใช้งานใหม่รอการอนุมัติ" ค้างอยู่ในกระดิ่งแจ้งเตือนของ Super Admin หลังผู้ใช้งานที่สมัครไว้ถูกลบไปแล้ว (เช่น ถูกปฏิเสธแล้วสมัครใหม่ซ้ำ หรือถูกลบโดย admin) — เดิมกดแจ้งเตือนแล้วขึ้น error 404 (ไม่พบผู้ใช้งาน) เพราะระบบลบผู้ใช้งานโดยไม่ได้ลบแจ้งเตือนที่ค้างอยู่ด้วย แก้โดยลบแจ้งเตือนที่เกี่ยวข้องพร้อมกันทุกจุดที่มีการลบผู้ใช้งาน พร้อมล้างแจ้งเตือนเก่าที่ค้างอยู่ในระบบออกแล้ว' },
      ]
    },
    {
      version: '2569.09.25.b',
      date: '25 กันยายน 2569 (build b)',
      changes: [
        { type: 'fix', text: 'แก้ไขระบบนับเวลาออกจากระบบอัตโนมัติ (Idle Timeout) ไม่ได้อ่านค่าที่ตั้งไว้จริงจากเซิร์ฟเวอร์ (เช่น จำนวนนาทีก่อนออกจากระบบ, เปิด/ปิดนับถอยหลัง) มาตั้งแต่แก้ไขให้ต้อง login ก่อนถึงจะเรียก API ตั้งค่าได้ — ใช้ค่าเริ่มต้นของระบบ (15 นาที) แทนเสมอโดยไม่มีใครสังเกตเห็น เพราะจุดที่ดึงค่ามาใช้ทำงานตั้งแต่ตอนเปิดแอปก่อน login เสมอ แก้โดยดึงค่าซ้ำอีกครั้งหลังจาก login สำเร็จแน่นอนแล้ว' },
      ]
    },
    {
      version: '2569.09.25.a',
      date: '25 กันยายน 2569 (build a)',
      changes: [
        { type: 'feature', text: 'หน้า "สิทธิ์การเข้าถึงหน้า" — เพิ่มตาราง "สิทธิ์เพิ่ม/แก้ไขข้อมูล" แยกจากสิทธิ์เข้าหน้าเดิม ครอบคลุมหน้าจัดการตัวชี้วัด/จัดการผู้ใช้งาน/ประกาศระบบ ให้ Super Admin เปิด/ปิดสิทธิ์ "เพิ่ม" และ "แก้ไข" ข้อมูลแยกจากกันต่อ Role ได้ (สิทธิ์ "ลบ" ยังคงจำกัดเฉพาะ Super Admin เสมอ ไม่มีข้อยกเว้น)' },
        { type: 'fix', text: 'แก้ไขจุดบกพร่องที่ทำให้การเปิดสิทธิ์เข้าหน้าให้ Role อื่น (จากฟีเจอร์ "สิทธิ์การเข้าถึงหน้า") ไม่ทำงานจริงในบางหน้า (ประกาศระบบ, ตั้งค่าระบบ, ผู้ใช้งานออนไลน์, จัดการข้อมูล KPI, +KPI ปีงบประมาณใหม่, ประวัติการใช้งาน, จัดการตัวชี้วัด) เนื่องจากหน้าเหล่านี้มีการตรวจสอบสิทธิ์แบบเดิมซ้ำอยู่ภายในอีกชั้นหนึ่ง' },
      ]
    },
    {
      version: '2569.09.24.d',
      date: '24 กันยายน 2569 (build d)',
      changes: [
        { type: 'feature', text: 'เพิ่มหน้า "สิทธิ์การเข้าถึงหน้า" (super_admin) — เปิด/ปิดการเข้าถึงแต่ละหน้าของระบบแยกตาม Role ได้ทั้ง 8 Role (ยกเว้น super_admin ที่เข้าได้ทุกหน้าเสมอ) ครอบคลุมทั้งเมนูที่มองเห็นและสิทธิ์เรียกใช้งาน API จริง (ไม่ใช่แค่ซ่อน/แสดงเมนู) — ตั้งค่าเริ่มต้นให้ตรงกับพฤติกรรมเดิมของระบบทุกประการ จึงไม่กระทบผู้ใช้งานเดิมจนกว่า super_admin จะปรับแก้เอง' },
      ]
    },
    {
      version: '2569.09.24.c',
      date: '24 กันยายน 2569 (build c)',
      changes: [
        { type: 'fix', text: 'แก้ไขลงทะเบียนผ่าน ThaiD/ProviderID (สแกน QR รับ Token) ไม่สามารถบันทึกได้ ขึ้น "Token หมดอายุ" ทุกครั้งแม้กรอกฟอร์มเสร็จเร็ว — สาเหตุคือ Token ถูกลบออกจากระบบทันทีตั้งแต่ตอนโหลดหน้าลงทะเบียนเพื่อดึงข้อมูลมาเติมในฟอร์ม (pre-fill) ทำให้ตอนกดปุ่ม "บันทึก" หา Token เดิมไม่เจอเสมอ แก้โดยให้ Token คงอยู่จนกว่าจะถูกใช้จริงตอนบันทึกข้อมูลสำเร็จเท่านั้น (ยังคงอายุ 10 นาทีและใช้ได้ครั้งเดียวเหมือนเดิม เพื่อป้องกันการนำกลับมาใช้ซ้ำ)' },
      ]
    },
    {
      version: '2569.09.24.b',
      date: '24 กันยายน 2569 (build b)',
      changes: [
        { type: 'fix', text: 'หน้าบันทึกผลงานตัวชี้วัด: ย้ายกล่องข้อความ "กรุณาเลือกตัวกรองก่อนดูข้อมูล" จากแถบหัวข้อด้านบน มาไว้ใต้แถวหัวตารางสีเขียวแทน จัดให้อยู่กึ่งกลางพื้นที่จอ ทำให้แถบหัวข้อโล่งขึ้นและข้อความอยู่ในตำแหน่งที่เห็นชัดเจนกว่าเดิม' },
      ]
    },
    {
      version: '2569.09.24.a',
      date: '24 กันยายน 2569 (build a)',
      changes: [
        { type: 'fix', text: 'หน้าบันทึกผลงานตัวชี้วัด: ย้ายปุ่ม "View ของฉัน" มาต่อท้ายปุ่ม "คำแนะนำ" ให้อยู่แถวบนสุดเสมอ (เดิมอยู่ในแถวตัวกรองที่พับซ่อนได้ ทำให้บางทีหาไม่เจอ) พร้อมเปลี่ยนวิธีคำนวณตำแหน่งกล่อง dropdown เป็นแบบคำนวณสดตามขอบจอจริง แทนการกำหนดชิดซ้าย/ขวาตายตัว ทำให้กล่องไม่ล้นออกนอกจอไม่ว่าปุ่มจะอยู่ตำแหน่งไหนของหน้าจอ' },
      ]
    },
    {
      version: '2569.09.23.g',
      date: '23 กันยายน 2569 (build g)',
      changes: [
        { type: 'feature', text: 'แบบฟอร์ม "ส่งข้อมูลเข้า HDC" (หน้าจัดการข้อมูล KPI): เพิ่มตัวกรอง "หมวดหมู่หลัก" และ "หน่วยงาน" พร้อมเพิ่ม 2 คอลัมน์นี้ในตารางตรวจสอบ ทำให้เห็นได้ทันทีว่าแต่ละตาราง export เป็นของหมวดหมู่/หน่วยงานใด และกรองหาตารางที่ต้องการส่งเข้า HDC ได้ง่ายขึ้นเมื่อมีตัวชี้วัดจำนวนมาก' },
      ]
    },
    {
      version: '2569.09.23.f',
      date: '23 กันยายน 2569 (build f)',
      changes: [
        { type: 'feature', text: 'เพิ่มปุ่ม "ส่งออก PDF" ในหน้าคู่มือการใช้งานและหน้าผังกระบวนการ SOP — คู่มือส่งออกเฉพาะหัวข้อที่สิทธิ์ปัจจุบันเห็นได้ (1 หัวข้อขึ้นไปต่อหน้า) ส่วน SOP ส่งออกผังกระบวนการครบทุกระบบและทุก sub-flow (1 หน้าต่อระบบ/sub-flow พร้อมคำอธิบายและสัญลักษณ์) ไฟล์ PDF ที่ได้เป็นภาพความละเอียดสูงจากหน้าจอจริง อ่านภาษาไทยได้ครบถ้วนชัดเจน' },
      ]
    },
    {
      version: '2569.09.23.e',
      date: '23 กันยายน 2569 (build e)',
      changes: [
        { type: 'feature', text: 'Export ข้อมูล KPI ลงตารางรายตัวชี้วัด: ตอนนี้ export ครบทุกหน่วยบริการ (hospcode) ที่อยู่ในขอบเขต "ประเภทหน่วยบริการ" (evaluation_mode/ประเภทหน่วยบริการที่ตั้งไว้) ของตัวชี้วัดเสมอ แม้ยังไม่มีใครคีย์ผลงานตัวชี้วัดหลักหรือตัวชี้วัดย่อยเลยก็ตาม — เดิมจะไม่มีแถวเลยถ้ายังไม่มีข้อมูล ทำให้ตารางไม่รู้ว่าหน่วยบริการไหนยังไม่รายงาน ตอนนี้จะเห็นแถวว่างรอไว้ครบทุกหน่วยบริการในขอบเขต' },
      ]
    },
    {
      version: '2569.09.23.d',
      date: '23 กันยายน 2569 (build d)',
      changes: [
        { type: 'fix', text: 'แก้ไข dropdown "View" และ "จัดการตัวชี้วัด" ในหน้าบันทึกผลงานตัวชี้วัด ล้นเข้าไปซ้อนทับ sidebar เมนูด้านซ้าย — สาเหตุคือกล่องตัวเลือกกางไปทางซ้าย (right-0) จากปุ่มที่อยู่ใกล้ขอบซ้ายของพื้นที่เนื้อหา เปลี่ยนให้กางไปทางขวาแทน (left-0) จึงไม่ล้นเข้าไปในพื้นที่ sidebar อีก' },
      ]
    },
    {
      version: '2569.09.23.c',
      date: '23 กันยายน 2569 (build c)',
      changes: [
        { type: 'fix', text: 'แก้ไข dropdown "View" (filter ที่บันทึกไว้) ในหน้าบันทึกผลงานตัวชี้วัด แสดงผลถูกซ้อนทับอยู่หลังตาราง — สาเหตุคือกล่อง filter ด้านบนใช้ z-index ต่ำกว่าหัวตารางแบบ frozen ทำให้เมื่อทั้งคู่ sticky พร้อมกันหัวตารางจะบังตัวเลือกใน dropdown' },
      ]
    },
    {
      version: '2569.09.23.b',
      date: '23 กันยายน 2569 (build b)',
      changes: [
        { type: 'fix', text: 'ส่งออกผลงานตัวชี้วัดย่อยล้มเหลวเมื่อชื่อข้อย่อยยาวเกิน 60 ตัวอักษรและตัดจบพอดีที่ช่องว่าง (error "Unknown column...in INSERT INTO") — MySQL ปฏิเสธชื่อคอลัมน์ที่ลงท้ายด้วยช่องว่าง แก้โดยตัดช่องว่างท้ายชื่อออกหลังตัดความยาว' },
        { type: 'feature', text: 'หน้าจัดการตัวชี้วัดย่อย: เพิ่มปุ่ม "นำเข้าจาก Excel" พร้อม template ดาวน์โหลด — สร้างตัวชี้วัดย่อยหลายรายการพร้อมกันได้จากไฟล์ Excel (รูปแบบเดียวกับการนำเข้าตัวชี้วัดหลัก)' },
      ]
    },
    {
      version: '2569.09.23.a',
      date: '23 กันยายน 2569 (build a)',
      changes: [
        { type: 'feature', text: 'การอนุมัติ/ส่งคืนแก้ไข/ปลดล็อคตัวชี้วัด (รวมถึงอุทธรณ์) มีผลไปถึงผลงานตัวชี้วัดย่อยด้วย — เดิมข้อมูลย่อยไม่เคยถูกล็อคตามตัวชี้วัดหลักเลย ทำให้แก้ไขได้แม้ตัวชี้วัดหลักอนุมัติแล้ว ตอนนี้ล็อค/ปลดล็อคพร้อมกันเสมอ (super_admin ยังแก้ไขได้แม้ล็อค)' },
        { type: 'feature', text: 'หน้าจัดการตัวชี้วัด: เพิ่มตัวเลือก "ส่งออกผลงานจากตัวชี้วัดย่อย" — เมื่อเปิดใช้ การส่งออกตาราง KPI จะสร้างคอลัมน์ตามชื่อตัวชี้วัดย่อยแต่ละข้อ (แทนคอลัมน์รายเดือน) โดยดึงค่าจากผลงานย่อยโดยตรง ชื่อตารางยังคงอิง table_process ของตัวชี้วัดหลักเหมือนเดิม' },
      ]
    },
    {
    ...
    },
    {
      ...
      changes: [
        { type: 'feature', text: 'ฟอร์มลงทะเบียนผ่าน ThaID/ProviderID: เพิ่มกล่องแสดงข้อมูล JSON ที่ถอดรหัสได้จาก token (ชื่อ/นามสกุล/เลขบัตรประชาชน/อำเภอ/หน่วยบริการ) ให้ผู้ใช้ตรวจสอบก่อน แล้วกดปุ่ม "นำข้อมูลนี้ไปใช้ในฟอร์ม" เอง แทนการ auto-fill ทันทีแบบเดิม — เพิ่มความมั่นใจว่าข้อมูลที่กรอกอัตโนมัติถูกต้องก่อนบันทึก' },
      ]
    },
    {
    ...
    },
    {
      ...
      changes: [
        { type: 'fix', text: 'คู่มือการใช้งาน: แก้ role gate ไม่ตรงกับ sidebar จริง 3 จุด — "จัดการผู้ใช้งาน" เปิดเฉพาะ admin_ssj/super_admin ทั้งที่ sidebar จริงเปิดให้ admin_cup/admin_hos/admin_sso ด้วย, "จัดการข้อมูล KPI" กับ "แจ้งเตือนการบันทึก KPI" เปิดให้ admin_ssj ทั้งที่ sidebar จริงเป็น super_admin เท่านั้น' },
        { type: 'feature', text: 'คู่มือการใช้งาน: เพิ่มเนื้อหาใหม่ครบ 6 หัวข้อที่ไม่เคยมีในคู่มือเลย — ผังกระบวนการ SOP, จัดการตัวชี้วัด, ประวัติการใช้งาน, KPI ปีงบประมาณใหม่, ประกาศระบบ, Error Logs' },
        { type: 'improve', text: 'คู่มือการใช้งาน: เรียงเลขหัวข้อใหม่ทั้งหมด 1-22 ให้ต่อเนื่องไม่ชนกัน + เมนู sidebar เรียงลำดับให้ตรงกับเนื้อหาจริง + ชื่อเมนูใส่เลขกำกับให้ตรงกับหัวข้อในรายละเอียดทุกจุด' },
        { type: 'improve', text: 'build.bat: เพิ่มขั้นตอน docker builder prune -af อัตโนมัติหลัง deploy สำเร็จทุกครั้ง กัน disk เต็มจาก image layer สะสม' },
      ]
    },
    {
      version: '2569.09.22.b',
      date: '22 กันยายน 2569 (build b)',
      changes: [
        { type: 'improve', text: 'ตารางบันทึกผลงานตัวชี้วัด: ย้ายคอลัมน์ "จัดการ" มาอยู่หน้า "ชื่อตัวชี้วัด" (ต่อจากหมวดหมู่หลัก) + ขยายคอลัมน์ที่ตรึง (frozen) จาก 4 เป็น 8 คอลัมน์ครบถึง "ร้อยละ" เหลือเฉพาะ 12 เดือนที่เลื่อนแนวนอนได้' },
        { type: 'feature', text: 'ตารางบันทึกผลงานตัวชี้วัด: เลื่อนอัตโนมัติไปยัง 3 เดือนล่าสุดที่มีผลงานเมื่อเปิดหน้า ยังเลื่อนดูเดือนอื่น ต.ค.-ก.ย. ได้ตามปกติ' },
        { type: 'feature', text: 'ตัวกรองทุกช่อง (อำเภอ/ประเภท รพ./หน่วยบริการ/สถานะ/หน่วยงาน/ตัวชี้วัดของ/ประเภทตัวชี้วัด): เพิ่ม checkbox "เลือกทั้งหมด" กดครั้งเดียวเลือกทุกตัวเลือก กดซ้ำยกเลิกทั้งหมด' },
        { type: 'fix', text: 'Profile dropdown (มุมขวาบน) ถูกตารางทับเวลาเปิดพร้อมกัน — ย้ายเป็น fixed position นอก stacking context ของ header (เหมือน pattern ที่ใช้กับ Notification dropdown)' },
        { type: 'improve', text: 'ข้อความ "กรุณาเลือกตัวกรองก่อนดูข้อมูล" ย้ายจากบังตารางทั้งหมด มาเป็น badge กระพริบอยู่กึ่งกลาง header ระหว่างชื่อหน้ากับปุ่ม "จัดการตัวชี้วัด" — หายไปหลังค้นหาสำเร็จ กลับมาใหม่หลังล้างตัวกรอง' },
        { type: 'improve', text: 'ข้อความ "ข้อมูลผลงานที่คีย์...เป็นข้อมูลสะสม" เปลี่ยนเป็นโทนสีแดงกระพริบช้าๆ ให้สังเกตเห็นง่ายขึ้น' },
      ]
    },
    {
      version: '2569.09.22',
      ...
      changes: [
        { type: 'fix', text: 'แก้ไข ThaiD/ProviderID login ใช้งานไม่ได้ในระบบจริง — สาเหตุจาก security hardening รอบก่อนหน้าตรวจสอบลายเซ็น JWT เข้มเกินจริง (DGA เซ็นด้วย RS256 ไม่ใช่ HS256 ตามที่คาดไว้) และ ISS allowlist ของ ProviderID ที่ไม่เคยทดสอบกับ token จริง — คืน fallback decode + เปลี่ยน allowlist เป็น log-only' },
        { type: 'fix', text: 'แก้ป้ายชื่อ provider ผิด — เข้าสู่ระบบด้วย ProviderID แต่ popup/แจ้งเตือนขึ้นเป็น "ThaID" ทั้ง popup สำเร็จและ popup แจ้ง error เพราะ iss ของ token จริงไม่ตรงกับที่คาดไว้ในการตรวจจับ provider' },
      ]
    },
    {
      version: '2569.09.04',
