...
taskkill /FI "WINDOWTITLE eq KHUPS-KPI-API [DEV :3000]*" /F >nul 2>&1
taskkill /FI "WINDOWTITLE eq KHUPS-KPI-WEB [DEV :4200]*" /F >nul 2>&1
...
start "KHUPS-KPI-API [DEV :3000]" cmd /k "cd /d %~dp0api && set NODE_ENV=development && npm run dev"

REM --- Start Frontend with ng serve on port 4200 (new window) ---
echo [2/2] Starting Frontend with ng serve on port 4200...
start "KHUPS-KPI-WEB [DEV :4200]" cmd /k "cd /d %~dp0frontend && npm run dev"
