...
### คำขอ
ผู้ใช้ส่งภาพหน้าจอหน้า "จัดการตัวชี้วัด" พบว่า modal "แก้ไขข้อมูล" แสดงผลไม่ครบ ส่วนบนของ modal (ชื่อหัวข้อ
"แก้ไขข้อมูล") ถูก header ด้านบนของระบบ (โลโก้/เมนูแฮมเบอร์เกอร์/แจ้งเตือน) บังทับอยู่

### สาเหตุที่ยืนยันจากการอ่านโค้ดจริง
เป็น **regression ที่เกิดจากงานแก้บั๊กเมนู slide มือถือก่อนหน้านี้ในวันเดียวกัน** — ตอนนั้นยกระดับ `<header>` จาก
`z-40` เป็น `z-[65]` แบบถาวร (ไม่มีเงื่อนไข) เพื่อให้อยู่เหนือฉากหลังมืดของเมนู slide มือถือ (`z-[60]`) เสมอ แต่
**modal ทั่วทั้งระบบ (เพิ่ม/แก้ไขข้อมูลแทบทุกหน้า) ใช้ `z-50` เป็นมาตรฐานเดียวกันหมด** — พอ header ถูกยกเป็น
`z-[65]` แบบถาวร (65 > 50) จึงไปทับหน้า modal ทุกจุดที่ใช้ `z-50` โดยไม่ตั้งใจ ทั้งที่ modal ควรอยู่เหนือ header
เสมอ (พฤติกรรม modal มาตรฐานที่ต้องบัง/บล็อกทุกอย่างรวม header ด้วย)

### วิธีแก้
`frontend/src/app/layout/layout.html` — เปลี่ยน z-index ของ `<header>` จากค่าคงที่ `z-[65]` เป็นแบบมีเงื่อนไข
ผ่าน `[ngClass]`: `isMobileMenuOpen ? 'z-[65]' : 'z-40'` — ตอนเมนู slide มือถือปิดอยู่ (สถานะปกติ 99% ของเวลา
รวมถึงตอน modal เปิดอยู่) header จะกลับไปที่ `z-40` เดิม (ต่ำกว่า modal `z-50` ทุกจุด) ตอนเมนู slide มือถือเปิด
จริงๆ ค่อยยกเป็น `z-[65]` ชั่วคราวเพื่อให้ยังกดปิดเมนูได้ตามที่ตั้งใจไว้เดิม

### ทดสอบยืนยัน (Playwright, ทั้ง 2 สถานะ)
- Modal เปิด (มือถือปิดเมนู): `headerZ=40`, `modalZ=50` — modal แสดงครบถูกต้อง ไม่มีส่วนไหนถูกบัง
- เมนู slide มือถือเปิด: `headerZ=65` (ยกระดับถูกต้อง) — กดลิงก์เมนูยัง navigate สำเร็จตามปกติ (ไม่มี regression
  ย้อนกลับไปที่บั๊กเดิม)
...
### คำขอ
ต้องการปรับแก้ไขเมนู slide บางหน้าบางครั้ง เมื่อมีการซ่อนเมนู จากปุ่มแฮมเบอเกอร์ หรือเปิดหน้าอื่นๆ แล้ว
ไม่สามารถเรียกแถบเมนูกลับมาได้ เมื่อกดปุ่มแฮมเบอเกอร์แล้วไม่แสดงแถบเมนูเลย

### สาเหตุที่ยืนยันจากการอ่านโค้ดจริง
- `layout.ts` ใช้ตัวแปร **`isSidebarOpen` ตัวเดียวกัน** ควบคุมทั้ง 2 เรื่องที่ไม่เกี่ยวข้องกัน:
  1. จอใหญ่ (desktop): เปิด/ปิด sidebar (`w-0` vs `w-16`/`w-64`)
  2. จอมือถือ: เปิด/ปิดเมนู slide dropdown + ฉากหลังมืด (backdrop)
- ระบบ Focus Mode (โหมดเต็มพื้นที่ตอนกด "แก้ไขผลงาน"/"โหมดลบ" ในหน้า Dashboard) มีกลไกจำค่า `isSidebarOpen`
  ก่อนเข้าโหมด (`_prevSidebarOpen`) แล้วบังคับปิด sidebar ระหว่างโหมด และ **คืนค่าเดิมอัตโนมัติตอนออกจากโหมด** —
  กลไกนี้ออกแบบมาสำหรับจอใหญ่ (ให้พื้นที่แก้ไขเต็มจอ) แต่เพราะแชร์ตัวแปรเดียวกับเมนู slide มือถือ การเข้า-ออก
  โหมดแก้ไขบนจอมือถือจึงไปเขียนทับค่า `isSidebarOpen` ที่จริงๆ ควรถูกควบคุมด้วยปุ่มแฮมเบอร์เกอร์เท่านั้น ทำให้
  บางจังหวะเมนู slide เปิดเองโดยไม่มีใครกด หรือค่าที่ควรจะกลับมาเปิดได้ไม่ตรงกับที่ผู้ใช้กดจริงอีกต่อไป
- ทดสอบซ้ำหลายรูปแบบผ่าน Playwright (mobile viewport, สุ่ม tap เร็ว, เข้า-ออกโหมดแก้ไขซ้ำหลายรอบ, เปลี่ยนหน้า
  สลับกับเปิดเมนู) ยืนยันว่า pattern การใช้ตัวแปรร่วมกันนี้เป็นจุดเสี่ยงจริงที่ทำให้สถานะเพี้ยนได้ในบางจังหวะ แม้จะ
  ไม่สามารถ pin จังหวะที่แน่นอน 100% ที่ผู้ใช้เจอ แต่เป็นสาเหตุเชิงสถาปัตยกรรมที่ชัดเจนและแก้ได้ตรงจุด

### วิธีแก้
แยกตัวแปรควบคุมเมนู slide มือถือออกมาต่างหาก (`isMobileMenuOpen`) ไม่ให้ใช้ร่วมกับ `isSidebarOpen` อีกต่อไป:
- `toggleSidebar()` (มือถือ) / `closeSidebar()` / auto-close ตอนเปลี่ยนหน้า (`NavigationEnd`) → ควบคุม
  `isMobileMenuOpen` เท่านั้น
- Focus Mode subscribe → ยังคงแตะแค่ `isSidebarOpen`/`isSidebarCollapsed` (จอใหญ่) เหมือนเดิม แต่**ไม่แตะ
  `isMobileMenuOpen` เด็ดขาด** แล้ว
- `layout.html`: backdrop มืด + กล่องเมนู slide เปลี่ยนมาอ่านจาก `isMobileMenuOpen` แทน `isSidebarOpen` —
  desktop aside ยังคงใช้ `isSidebarOpen`/`isSidebarCollapsed` เหมือนเดิม ไม่กระทบ
- ทดสอบยืนยันผ่าน Playwright: เปิด/ปิดเมนูซ้ำ, เข้า-ออกโหมดแก้ไข 3 รอบติด, สลับหน้า — กดแฮมเบอร์เกอร์แล้วเมนู
  ขึ้นสำเร็จทุกครั้ง ไม่มีครั้งไหนค้าง

### ไฟล์ที่แก้ไข
- `frontend/src/app/layout/layout.ts`, `frontend/src/app/layout/layout.html`
- `CLAUDE.md` (อัปเดตส่วน Shared State ให้ตรงกับพฤติกรรมใหม่)

---

## 2569-09-26 — แก้เมนู slide บนมือถือถูกกล่องตัวกรองทับ กดเลือกเมนูไม่ได้

### คำขอ
หน้า dashboard บันทึกผลงานตัวชี้วัด กล่องตัวกรองซ้อนทับเมนู slide (มือถือ) ทำให้เลือกเมนูไม่ได้ — พร้อมภาพหน้าจอ
ประกอบ (มือถือ กดปุ่ม hamburger เปิดเมนูแล้วเห็นกล่องตัวกรองทับอยู่ด้านบน ไม่ดำมัวเหมือนพื้นหลังปกติ)

### สาเหตุที่ยืนยันจากการอ่านโค้ดจริง
- Layout mobile menu (`layout.html`): ฉากหลังมืด (backdrop) เดิม `z-40`, กล่องเมนู slide เดิม `z-50`
- Dashboard filter wrapper (`dashboard.html:9`) เป็น `sticky top-0 z-50` (ถูกปรับจาก z-30 เป็น z-50 ในงานก่อนหน้า
  เพื่อให้สูงกว่าหัวตาราง frozen ที่ z-40 ใน `dashboard.css`) — ทำให้ z-index **เท่ากับ** เมนู slide ของ layout พอดี
- เมื่อ z-index เท่ากัน กติกา CSS จะให้ธาตุที่มาทีหลังใน DOM ทับอยู่บน — กล่องตัวกรองของ dashboard (routed content)
  render หลังเมนู slide ของ layout เสมอ จึงทับเมนูจนกดไม่ทะลุ และเพราะตัวกรองมี z-index สูงกว่าฉากหลังมืด (z-40)
  ด้วย มันเลยไม่ถูกทำมืดแบบพื้นหลังปกติ (เห็นสว่างชัดทับอยู่)
- ตรวจแล้วพบว่ามีอีก 15 หน้าที่ใช้ pattern `sticky ... z-50` แบบเดียวกัน (kpi-manage, user-management,
  announcements, notifications, feedback, report ฯลฯ) — เป็นปัญหาเชิงระบบ ไม่ใช่แค่หน้า dashboard เดียว

### วิธีแก้
แก้ที่ระดับ layout (จุดเดียว ครอบคลุมทุกหน้า) แทนที่จะไล่ลด z-index ทีละหน้า:
- backdrop มืด: `z-40` → `z-[60]`
- กล่องเมนู slide: `z-50` → `z-[70]`
- header (ที่มีปุ่ม hamburger): `z-40` → `z-[65]` (ต้องยกตามด้วย ไม่งั้น backdrop ใหม่จะทับ header จนมืดไปด้วย
  ทั้งที่ header ต้องเห็น/กดได้ตลอดเวลาเพื่อปิดเมนู)
- ตรวจแล้วว่า z-index สูงสุดที่มีอยู่จริงในทุกหน้า (`dashboard.css` frozen header = 40) ยังต่ำกว่าค่าใหม่ทั้งหมด
  จึงไม่กระทบ dropdown/sticky อื่นๆ ที่มีอยู่เดิม
- ทดสอบผ่าน dev server + Playwright ที่ mobile viewport (390×844): เปิดเมนู → กล่องตัวกรองถูกทำมืดถูกต้อง →
  กดลิงก์เมนูสำเร็จ (navigate ไปหน้าอื่นได้จริง ไม่ใช่แค่มองเห็นแต่กดไม่ทะลุเหมือนเดิม)
